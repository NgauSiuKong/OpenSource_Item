...
<div class="pd-20">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="member_add('添加用户','member-add.html','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span> <span class="r">共有数据：<strong><?php echo ($list_stop['count']); ?></strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
            <?php if(is_array($list_stop['selArr'])): foreach($list_stop['selArr'] as $key=>$val): ?><tr class="text-c">
				<td><input type="checkbox" value="1" name=""></td>
				<td><?php echo ($val["id"]); ?></td>
                <td>
                    <a href="/VRAppItem/index.php/Backstage/User/userShow?id=<?php echo ($val["id"]); ?>">
                        <?php echo ($val["username"]); ?>
                    </a>
                </td>
				<td>
                    <?php if($val["status"] == 1): ?><span class="label label-success radius">男</span>
                    <?php else: ?>
                        <span class="label label-danger radius">女</span><?php endif; ?>               
                </td>
				<td><?php echo ($val["tel"]); ?></td>
				<td><?php echo ($val["email"]); ?></td>
				<td>
                    <?php if($val["status"] == 1): ?><span class="label label-success radius">会员用户</span>
                    <?php else: ?>
...
function member_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
        /*
        $.ajax({ 
                url:'/VRAppItem/index.php/Backstage/User/status',
                type:'post',
                dataType:'json',
                data:{id:id},
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
            $.ajax({ 
                url:'/VRAppItem/index.php/Backstage/User/delete',
                type:'post',
                dataType:'json',
                data:{id:id},
                success:function(msg)

...
<body>
<h2 style="text-align:center">添加"<?php echo ($class_info['c_title']); ?>"分类下的子分类</h2>
<div class="pd-20">
	<form action="/VRAppItem/index.php/Backstage/Class/classSonAdd" method="post" class="form form-horizontal" id="form-article-add">
        <input type="hidden" value="<?php echo ($class_info['c_id']); ?>" name="fid"/>
        <input type="hidden" value="<?php echo ($class_info['c_path']); ?>" name="path"/>
 		<div class="row cl">
			<label class="form-label col-2">分类标题：</label>
			<div class="formControls col-10">
				<input type="text" class="input-text" value="" placeholder="输入子分类标题" id="imageatitle" name="title">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-2">分类描述：</label>
			<div class="formControls col-10">
				<input type="text" class="input-text" value="" placeholder="输入子分类描述" id="imageadescribe" name="describe">
			</div>
		</div>
		<div class="row cl">

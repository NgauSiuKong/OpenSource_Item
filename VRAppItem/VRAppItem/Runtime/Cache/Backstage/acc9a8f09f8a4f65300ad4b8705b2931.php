...
<div class="pd-20">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" href="/VRAppItem/index.php/Backstage/CarouselFigure/add"><i class="Hui-iconfont">&#xe600;</i> 添加图片</a></span> <span class="r">共有数据：<strong><?php echo ($list['count']); ?></strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
                <?php if(is_array($list['selArr'])): foreach($list['selArr'] as $key=>$val): ?><tr class="text-c">
					...
					<td><a href="javascript:;" onClick="picture_edit('图库编辑','picture-show.html','10001')"><img width="100" class="picture-thumb" src="/VRAppItem/Public/<?php echo ($val["path"]); ?>"></a>
                    </td>
					<td class="text-l"><?php echo ($val["type"]); ?></td>
					<td class="text-c"><?php echo ($val["data"]); ?></td>
					<td><?php echo ($val["addtime"]); ?></td>
                    <td>
                        <i class="Hui-iconfont"><a href="/VRAppItem/index.php/Backstage/CarouselFigure/carfigSort?handle=pre&id=<?php echo ($val["id"]); ?>&oid=<?php echo ($val["oid"]); ?>">&#xe679;</a></i>
                        <i class="Hui-iconfont"><a href="/VRAppItem/index.php/Backstage/CarouselFigure/carfigSort?handle=next&id=<?php echo ($val["id"]); ?>&oid=<?php echo ($val["oid"]); ?>">&#xe674;</a></i>
                    </td>
					<td class="td-status">
                        <?php if($val["status"] == 1): ?><span class="label label-success radius">已发布</span>
                        <?php else: ?>
					...
					<td class="td-manage">
                        ...
                        <a class="ml-5" href="/VRAppItem/index.php/Backstage/CarouselFigure/edit?id=<?php echo ($val["id"]); ?>" title="编辑">
        ...
        <div style="float:right"><?php echo ($list['show']); ?></div>
	</div>
</div>
...
$('.table-sort').dataTable({
    ...
    function article_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({ 
                url:'/VRAppItem/index.php/Backstage/CarouselFigure/delete',
                type:'post',
                dataType:'json',
                data:{id:id},
    ...
    function article_stop(obj,id){
        layer.confirm('确认要下架吗？',function(index){
            $.ajax({ 
                url:'/VRAppItem/index.php/Backstage/CarouselFigure/status',
                type:'post',
                dataType:'json',
                data:{id:id},
                success:function(msg){ 
